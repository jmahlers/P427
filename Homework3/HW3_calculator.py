...
print("Max eigenvalue:", np.max(hil10eig))
print("Min eigenvalue:", np.min(hil10eig))
hil10inv = lag.inv(hil10)
product = hil10inv @ hil10
#print("Inv*hil", product)
error = np.absolute(product - np.identity(10))
print("Max error 1st method is:", np.max(error))
...
print("Max error 2nd method is:", np.max(error2))
